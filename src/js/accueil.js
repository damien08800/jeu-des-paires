...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (maxLength >= 3) {
    ...
    document.addEventListener('click', (evt) => {
      ...
      if (eventTarget.classList.contains('custom-arrows') && anti_spam) {
        anti_spam = !anti_spam;
  ...
  antispam2 = true;
  antispam3 = true;
  const play_button = document.querySelector('#play_button button');
  ...
  function loadSelectedManyCards() {
    ...
    if (savedManyCards){
      ...
      changeElements2[1].classList.add('active');
    }
  }
    ...
    items.forEach(it => {
        if (it.getAttribute('position') == 2) {
          ...
          if(div.classList.contains('disable')){
            available = true;
            play_button.classList.add('unavailable');
          }
          else{
            play_button.classList.remove('unavailable');
          }
        }
    });
    return available;
